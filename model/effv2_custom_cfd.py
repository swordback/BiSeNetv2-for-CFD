...
class GELayerS3(nn.Module):

    def __init__(self, in_chan, out_chan, exp_ratio=6):
        super(GELayerS3, self).__init__()
        mid_chan = in_chan * exp_ratio
        self.conv1 = ConvBNReLU(in_chan, out_chan, 3, stride=2, padding=1)
        self.conv2 = nn.Sequential(
            nn.Conv2d(
                out_chan, out_chan, kernel_size=1, stride=1,
                padding=0, bias=False),
            nn.BatchNorm2d(out_chan),
        )
        self.conv2[1].last_bn = True
...
if __name__ == "__main__":
    ...
    #  print(feat.size())
    #
    x = torch.randn(16, 3, 1024, 2048).cuda()
    model = BiSeNetV2(n_classes=19).cuda()
    outs = model(x)
    for out in outs:
        print(out.size())
    #  print(logits.size())
